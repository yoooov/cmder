...
local function set_prompt_filter()
	...
	-- get_lamb_color() return lamb_color or "\x1b[1;30;49m" -- Light Grey = Lambda Color
	-- uah = "┌─[" .. clink.get_env("USERNAME") .. "@" .. clink.get_env("COMPUTERNAME") .. "] "
    prompt = "┌─[{uah}]─[{cwd}" .. get_lamb_color() .. "]" .. version_control .. get_lamb_color() .. cr .. "{env}{lamb} \x1b[0m"
    prompt = string.gsub(prompt, "{uah}", uah)
    prompt = string.gsub(prompt, "{cwd}", cwd)
    prompt = string.gsub(prompt, "{env}", env)
    -- clink.prompt.value = string.gsub(prompt, "{lamb}", prompt_lambSymbol)
	pr_lamb = prompt_prefix_lambSymbol .. " " .. prompt_lambSymbol
	clink.prompt.value = string.gsub(prompt, "{lamb}", pr_lamb)
end
